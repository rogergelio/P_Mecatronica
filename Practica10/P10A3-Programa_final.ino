  ...
  switch(estado){
    ...
    case 8: //forzar adelante despues de esquivar derecha 
        if(U1>7 && U2 >7 && S1<200 && S2<200){
          ...
          break; 
       case 9: //forzar adelante despues de esquivar objeto a la izquierda
        if(U1>7 && U2 >7 && S1<200 && S2<200){
          driveRobot(-1, -1); 
          LCD_print("Forzar adelante", L_Diff1); 
          delay(400); 
...
void driveRobot(float LSignal, float RSignal){ //función princiapl del robot
...
void voltageSensor(){//Sensor de voltaje
...
void LCD_print(String mensaje, int L_Diff){ //Utilizamos este método para imprimir mensajes en la pantalla
  lcd.setCursor(0,0);
  lcd.print("                  ");
  lcd.setCursor(0,1);
  lcd.print("                  ");
